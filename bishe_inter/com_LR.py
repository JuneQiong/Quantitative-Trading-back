...
def getData(code):
    filename_day = 'D:/software/python/project/stock/Data/bishe/BT/total/ml_lr_test/day/'+ code +'.csv'
    filename_week = 'D:/software/python/project/stock/Data/bishe/BT/total/ml_lr_test/week/'+ code +'.csv'
...
def handleTestData(tickers):
    ...
    for i,ticker in enumerate(tickers):
        print(i,ticker)
        filename = 'D:/software/python/project/stock/Data/bishe/BT/total/ml_lr_test/day/'+ ticker +'.csv'
        df = pd.read_csv(filename)
        df = df.drop(['Unnamed: 0'], axis=1)
        # 获取XY
        length = len(df)
...
def plot(start,end):
    ...
    transactions = pd.read_csv('D:/software/python/project/stock/learn_envs/bishe_inter/transactions_LR.csv')
    ...
    if (transactions.empty):
        ...
        plt.suptitle(u'LR 买卖点回测结果 --> 收益率：' + str(prorate) + '%',fontsize = 14)

    plt.savefig('D:/software/python/project/stock/learn_envs/bishe_inter/result/LR_com.png')
    plt.show()
    plt.close()
...
def main():
    ...
    while d <= end:
        ...
        if num == 0:
            ...
            filename = 'D:/software/python/project/stock/Data/bishe/BT/total/ml_lr_test/day/' + code + '.csv'
            if (int(f) in pdatas.trade_date.values):
                # print('time is exits')
                i = pdatas[pdatas.trade_date.values == int(f)].index.values
                i = int(str(i).strip('[]'))
                ...
                # print('check:',pdatas.position[i-1])
                if ((pdatas.K[i - 2] > pdatas.D[i - 2]) & (pdatas.K[i] < pdatas.D[i])):
    ...
    endtime=time.time()
    if transactions.empty:
        transactions.to_csv('transactions_LR.csv')
        print('没有买卖记录，盈利为0')
    ...
    # ----接口
    url = 'D:/software/python/project/stock/learn_envs/bishe_inter/result/LR_com.png'
