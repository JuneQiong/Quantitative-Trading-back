...
def model_ml(code,day,X_train,Y_train):
    ...
    # 保存模型
    joblib.dump(clf, code+"svm_model-"+ day+".m")
    return clf
...
def visualize(code,predict_time,Y_date,Y_test, Y_pre, Y_apply):
    ...
    plt.suptitle('\"' + code + '-' + str(name[0]) + u'\"的 SVM 算法预测结果',fontsize = 14)
    # ----接口
    plt.savefig('D:/software/python/project/stock/learn_envs/bishe_inter/result/SVM_pre.png')
    plt.show()

@app.route('/svmpre', methods=["GET"])  
def main():
    ...
    try:
        # 载入模型
        # 载入模型
        model = joblib.load(code+"svm_model-"+ day+".m")
        print('model1',model)
    except:
    ...
    visualize(code,predict_time,Y_date,Y_test, Y_pre, Y_apply)
     # ----接口
    url = 'D:/software/python/project/stock/learn_envs/bishe_inter/result/SVM_pre.png'
